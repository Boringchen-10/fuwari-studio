...
解压整个文件夹到自己有写入权限的位置（如桌面），双击 `Blog Studio v0.1.3.exe`。不要单独复制 EXE。程序附带 Electron、Node 22 和 Fuwari 依赖，无需另外安装 Node、Git 或包管理器。首次启动会准备环境，可能需要数分钟和约数 GB 可用空间。

程序数据位于 `%APPDATA%\Blog Studio`。通过「网站与版本」或“项目”菜单可以创建、打开和切换兼容网站。首次运行 v0.1.3 会自动查找旧版 `Fuwari Studio Data` 并导入原有网站路径、工作台设置和加密连接资料。普通 Fuwari 项目需先适配 `src/site-settings.json`。
...
3. 在“服务器连接”中新建一个命名配置，填写主机、SSH 端口、用户名、密码或私钥、网站 URL 和网站目录。可以为同一网站保存多台服务器。
4. 点击“测试连接”，与管理员提供的 SSH SHA256 指纹核对，再保存。服务器凭据使用 Windows 本机加密，不放入博客目录或分发包。
5. 在“发布中心”勾选一台或多台 SFTP 服务器，也可同时勾选 GitHub Pages，再点击“构建并发布”。
...
GitHub Pages 配置步骤见 `resources/app/GITHUB.md`。v0.1.3 新增多网站、版本时间线、多 SFTP 服务器和多目标发布。每次构建会在当前网站 `.local-admin` 目录下保留按时间和网站名称标记的发布包及源码快照，可从「网站与版本」页面基于快照创建副本。
