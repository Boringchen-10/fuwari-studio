...
const {validateDescriptor,resolveProjectPaths} = localRequire('../project-standard.cjs');
const descriptor = validateDescriptor(JSON.parse(process.env.BLOG_STUDIO_DESCRIPTOR || '{}'));
const {getAdapter} = localRequire('../adapters/index.cjs');
const adapter = getAdapter(descriptor.base.id);
const projectPaths = resolveProjectPaths(root, descriptor);
const welcome = process.env.BLOG_STUDIO_WELCOME === '1';
const postsRoot = projectPaths.content;
const localRoot = welcome ? path.resolve(process.env.BLOG_STUDIO_LOCAL_ROOT || path.join(root, '.local-admin-welcome')) : path.join(root, '.local-admin');
...
const releasesRoot = projectPaths.output;
const settingsFile = projectPaths.settings;
const uploadPath = path.relative(projectPaths.public, projectPaths.assets).replaceAll('\\', '/');
if (!uploadPath || uploadPath.startsWith('../') || uploadPath === '..') throw new Error('资源目录必须位于公开资源目录内');
const uploadUrlBase = `/${uploadPath}`;
...
const browserStudioSettings = {theme:'system',hue:150,device:'desktop',autosaveDelay:700,cacheDir:''};
...
function desktopCall(action, data = {}) {
  if (!process.send) fail('此功能需要在 Blog Studio 桌面程序中使用', 400);
...
const snapshotFiles=['blog-studio.json','src','public','package.json','pnpm-lock.yaml','astro.config.mjs','tailwind.config.cjs','postcss.config.mjs','svelte.config.js','tsconfig.json','LICENSE'];
...
async function allVersions(){const result=await listVersions();const known=new Set(result.map(item=>item.id));await fs.mkdir(releasesRoot,{recursive:true});for(const id of await fs.readdir(releasesRoot)){if(known.has(id))continue;try{const stat=await fs.stat(path.join(releasesRoot,id));if(stat.isDirectory())result.push({id,createdAt:stat.birthtime.toISOString(),siteName:'旧版发布包',siteUrl:'',status:'built',output:path.join(releasesRoot,id),source:'',legacy:true});}catch{}}return result.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));}
...
async function buildRelease(forDeployment=false) {
  ...
  try {
    ...
    const out = path.join(releasesRoot,id);
    await createVersionSnapshot(id,createdAt,settings);
    for(const args of adapter.buildSteps(root,out))await runBuildCommand(args);
...
async function api(req,res,url) {
  ...
  if (route === '/api/desktop' && req.method === 'GET') return json(res, {enabled:Boolean(process.send),version:'0.2.0',project:welcome?'':root,welcome,standardVersion:descriptor.standardVersion,base:descriptor.base});
  if (route === '/api/desktop/settings' && req.method === 'GET') return json(res, process.send ? await desktopCall('studio-settings-get') : browserStudioSettings);
  if (route === '/api/desktop/settings' && req.method === 'PUT') {
    const input=await body(req);
    if(process.send)return json(res,await desktopCall('studio-settings-save',input));
    Object.assign(browserStudioSettings,input);return json(res,browserStudioSettings);
  }
  ...
  if (route === '/api/projects' && req.method === 'GET') {
    if(process.send)return json(res,await desktopCall('projects-get'));
    const settings=JSON.parse(await fs.readFile(settingsFile,'utf8'));
    return json(res,{current:root,projects:[{path:root,name:settings.title||path.basename(root),subtitle:settings.subtitle||'',exists:true,standard:true,base:adapter.displayName,message:'已符合 Blog Studio Standard 1.0'}],welcome:false});
  }
  if (route === '/api/projects/open' && req.method === 'POST') return json(res, await desktopCall('project-open'));
  if (route === '/api/projects/inspect' && req.method === 'POST') return json(res, await desktopCall('project-inspect'));
  if (route === '/api/projects/import' && req.method === 'POST') return json(res, await desktopCall('project-import',await body(req)));
  if (route === '/api/projects/create' && req.method === 'POST') return json(res, await desktopCall('project-create',await body(req)));
  ...
  if (route === '/api/connections' && req.method === 'GET') return json(res, process.send ? await desktopCall('connections-get') : {sftpProfiles:[],sftp:{},github:{}});
  ...
  if(route==='/api/bootstrap' && req.method==='GET') {
    if(welcome)return json(res,{csrf,welcome:true,settings:{title:'我的网站',subtitle:'记录生活，分享热爱',name:'博主',bio:'欢迎来到我的小小空间。',avatar:'',banner:'',hue:150,bannerEnabled:true,tocEnabled:true,navigation:[],links:[]},settingsRevision:'',posts:[],previewOrigin:'',previewReady:false,previewError:'',job,trash:[],descriptor});
    const settings=await readVersioned(settingsFile);
    return json(res,{csrf,welcome:false,settings:JSON.parse(settings.text),settingsRevision:settings.revision,posts:(await listPosts()).map(({body,relative,...post})=>post),previewOrigin,previewReady,previewError,job,trash:await trashList(),descriptor});
  }
  ...
  if(route.startsWith('/api/pages/')) {
    ...
    const file=projectPaths.pages[kind];const current=await readVersioned(file);
  ...
  if(route==='/api/upload' && req.method==='POST') {
    ...
    const dest=path.join(projectPaths.assets,filename);await fs.mkdir(path.dirname(dest),{recursive:true});
    await image.rotate().resize({width:2000,height:2000,fit:'inside',withoutEnlargement:true}).webp({quality:85}).toFile(dest);
    return json(res,{path:`${uploadUrlBase}/${filename}`});
  }
...
const server=http.createServer(async(req,res)=>{
  ...
  try {
    ...
    if(url.pathname==='/media') {
      ...
      if(source.startsWith(`${uploadUrlBase}/`))file=confined(projectPaths.assets,source.slice(uploadUrlBase.length+1));
...
if(!welcome){
  preview=spawn(process.execPath,adapter.previewArgs(root,previewPort),{cwd:root,windowsHide:true,env:{...process.env,ASTRO_TELEMETRY_DISABLED:'1'}});
  preview.stdout.on('data',previewLog);preview.stderr.on('data',previewLog);
  preview.on('error',error=>{previewError=error.message;});
  preview.on('close',code=>{previewReady=false;previewError=`本地预览已停止 (${code})，请重新打开 Blog Studio。`;});
}
