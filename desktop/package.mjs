...
const output=await packager({dir:base,out:path.resolve(base,'../release-v011'),name:'Fuwari Studio',appVersion:'0.1.1',platform:'win32',arch:'x64',overwrite:true,asar:false,prune:false,electronVersion:'40.10.6',electronZipDir:process.env.ELECTRON_ZIP_DIR,ignore:[/^\/(?!node_modules(?:\/|$)|main\.cjs$|runtime\.cjs$|deploy\.cjs$|github\.cjs$|GITHUB\.md$|package\.json$|README\.md$|NODE-LICENSE\.txt$).+/,/^\/node_modules\/(@electron|electron|electron-packager)(\/|$)/],extraResource:[template,path.join(staging,'runtime'),path.join(staging,'module-links.json'),path.join(staging,'使用说明.txt')],win32metadata:{CompanyName:'Fuwari Studio',FileDescription:'Fuwari Studio v0.1',ProductName:'Fuwari Studio',InternalName:'FuwariStudio'}});
